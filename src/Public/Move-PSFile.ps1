...
function Move-PSFile {
    ...
    if ($RenameDuplicate) {
        if (Test-Path -LiteralPath $destinationLocation) {
            $fileCounter = 1
            do {
    ...
    Move-Item -Path $Path -Destination $destinationLocation
}

Export-ModuleMember -Function Move-PSFile